...
function scheduleLazyReplay(tuning, scriptNonce) {
  ...
  const attach = () => {
    ...
    pending = true;
    attempts += 1;
    Sentry.lazyLoadIntegration("replayIntegration", scriptNonce).then((replayIntegration2) => {
      attached = true;
      pending = false;
      Sentry.addIntegration(replayIntegration2(tuning));
    }).catch(() => {
      pending = false;
...
function initSentryClient(opts) {
  ...
  initClientCore({
    ...
    eagerReplay: (tuning) => Sentry.replayIntegration({
