...
Sentry (`console.error` to Vercel logs). The hooks keep the SDK's own
`contexts.runtime.name` / `contexts.os.name` / `contexts.browser.name` ("node",
"Linux") readable — value-scrubbed, not key-redacted — so the `runtime.name` and
`os.name` tags stay usable for triage; `name` anywhere else, `contexts.device.name`
included, is still `[REDACTED]` (DECISIONS.md §18). Not covered: tokens in URL **paths**
